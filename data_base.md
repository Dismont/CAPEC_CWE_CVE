...
SQLite использует **"динамическая типизация"**, что означает, что тип данных может изменяться в зависимости от содержимого столбца. Однако при создании таблиц рекомендуется указывать типы для улучшения производительности и ясности.
...
SQLite автоматически преобразует типы данных на основе содержимого. Например:
- `VARCHAR(255)` → `TEXT`
- `INT` → `INTEGER`
- `FLOAT` → `REAL`
- `BOOLEAN` → `INTEGER` (0 = false, 1 = true)

Поэтому нет необходимости указывать длину для типа `TEXT` или точность для `REAL`.
...
CREATE TABLE products (
    ...
    name TEXT NOT NULL,                         -- Название продукта (обязательное поле)
    description TEXT,                           -- Описание продукта
    price REAL CHECK (price >= 0),              -- Цена продукта (не может быть меньше 0)
    quantity INTEGER DEFAULT 0,                 -- Количество товара (по умолчанию 0)
    created_at TEXT DEFAULT CURRENT_TIMESTAMP,  -- Время создания записи
    image BLOB                                  -- Изображение продукта
);
```
---
...
CPE-идентификаторы имеют формат
- `cpe:2.3:<part>:<vendor>:<product>:<version>:<update>:<edition>:<language>:<sw_edition>:<target_sw>:<target_hw>:<other>`.\
Каждый компонент имеет определенное значение или может быть указан как `*` (любое значение) или `-` (пустое значение).
